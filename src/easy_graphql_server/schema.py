...
from graphql.type.validate import (
    validate_schema,  # pylint: disable=no-name-in-module,import-error
)
from graphql.utilities import (
    get_introspection_query,  # pylint: disable=no-name-in-module,import-error
)

from . import exceptions, exposition, introspection
from .casing import Casing
from .context import ContextValue
from .conversion import to_graphql_argument, to_graphql_type
...
class Schema:
    ...
    def __init__(
        ...
        debug=False,
        casing=Casing.SNAKE,
        restrict_models_queried_fields=False,
        models_max_depth=None,
        models_limit=-1,
        models_allowed_lookups=None,
        models_disallowed_lookups=None,
    ):
    ...
    def get_documentation(self, with_descriptions=False):
        ...
        result = self.execute(
            get_introspection_query(
                descriptions=with_descriptions, directive_is_repeatable=True
            )
        )
        return result.data
    ...
    def expose(self, cls):
        ...
        if not inspect.isclass(cls):
            raise ValueError(
                "Parameter to `expose()` method should be a class, "
    ...
    def execute(
        ...
        self._requests_logger.info(
            json.dumps(
                {
                    "authenticated_user": authenticated_user.username
                    if authenticated_user is not None
                    else None,
                    "query": simpler_query[:64] + "..."
                    if len(simpler_query) > 64
                    else simpler_query,
                }
            )
        )
    ...
    def get_model_config(self, orm_model=None, name=None):
        ...
        for model_config in self.models_configs:
            if (
                model_config.orm_model_manager.orm_model == orm_model
                or model_config.name == name
            ):
                return model_config
        return None
    ...
    def as_django_view(self, with_graphiql=True, compute_user=True):
        ...
        return DjangoSchemaView(
            schema=self, with_graphiql=with_graphiql, compute_user=compute_user
        ).view
        ...
        self.methods[type_][name] = GraphQLField(
            ...
            resolve=self._make_callback(
                ...
                pass_graphql_selection=(
                    "graphql_selection"
                    if pass_graphql_selection is True
                    else pass_graphql_selection
                ),
                pass_graphql_path=(
                    "graphql_path" if pass_graphql_path is True else pass_graphql_path
                ),
                pass_authenticated_user=(
                    "authenticated_user"
                    if pass_authenticated_user is True
                    else pass_authenticated_user
                ),
    ...
    def _expose_from_subclass(self, subclass):
        ...
        if issubclass(subclass, exposition.ExposedModel):
            ...
            raise ValueError(
                "Parameter to `expose()` method should be a subclass of either "
    ...
    def _make_graphql_schema(self):
        ...
        return GraphQLSchema(
            query=GraphQLObjectType("Query", lambda: self.methods["query"])
            if self.methods["query"]
            else None,
            mutation=GraphQLObjectType("Mutation", lambda: self.methods["mutation"])
            if self.methods["query"]
            else None,
        )
            ...
            if isinstance(selection, FieldNode):
                ...
                result[name] = (
                    cls._get_graphql_selection(
                        selection.selection_set, fragments, visited_fragments
                    )
                ...
                result.update(
                    cls._get_graphql_selection(
                        ...
                        fragments,
                        visited_fragments | {selection.name.value},
                    )
                )
        # the end!
        return result
