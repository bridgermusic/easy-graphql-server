...
def generate_testcase(
    schema, graphql_path, base_test_class=DefaultBaseTestCase, django_environment=False
):
    """
    ...
    class TestCase(base_test_class):
        ...
        def _iterate_data(self, graphql_path):
            ...
            with open(json_path, "rt", encoding="utf-8") as json_file:
                ...
                for i, expected_output in enumerate(
                    ...
                    try:
                        json_list.append(json.loads(expected_output))
                    except json.decoder.JSONDecodeError as error:
                        raise ValueError(
                            f"Cannot decode JSON in `{json_path}`[{i}]: "
                            "{error}\n{expected_output}"
                        ) from error
            # load generated SQL
            ...
            # return iterator
            for i, (graphql, json_, sql) in enumerate(
                zip(graphql_list, json_list, sql_list)
            ):
        ...
        def run_test(self, *args, **kwargs):  # pylint: disable=unused-argument
            ...
            for index, (user, graphql, json_, sql) in self._iterate_data(graphql_path):
                ...
                if sql:
                    schema.check()
                    for orm_model_manager_class in set(
                        schema.orm_model_manager_classes
                    ):
    ...
    TestCase.__qualname__ = ".".join(
        TestCase.__qualname__.split(".")[:-1] + [class_name]
    )
...
class EgsError:
    ...
    message: str
    locations: List[EgsErrorLocatin]
    path: List[str]


class EgsOutputFactory:
    """Generate Django EGS expected output"""

    @staticmethod
    def create_data_template(
        data: Optional[Mapping[str, Any]] = None
    ) -> Mapping[str, Any]:
        """Return EGS response ``data`` attribute"""
        return {"data": data}
    ...
    def bake_egs_output(
        cls,
        data: Optional[Mapping[str, Any]] = None,
        errors: Optional[List[EgsError]] = None,
    ) -> Mapping[str, Any]:
        """Bake a customized EGS output suited for Django integration"""
        result = {}

        result.update(cls.create_data_template(data))

        if errors is not None:
            result.update({"errors": [asdict(error) for error in errors]})
        return result
