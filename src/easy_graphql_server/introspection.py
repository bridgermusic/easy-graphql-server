...
def get_method_arguments(method, exclude=None):
    ...
    result = {
        arg: args_count - i > default_args_count for i, arg in enumerate(signature.args)
    }
    if exclude:
        for arg in exclude:
            result.pop(arg)
...
def validate_class_attributes_against_method_arguments(
    cls, method, excluded_arguments=None, message=""
):
    """
    ...
    for required_argument in required_arguments:
        if required_argument not in attributes:
            raise ValueError(
                f"Attribute `{required_argument}` should be present on "
                f"class `{cls}`{message}"
            )
    for attribute in attributes:
        if attribute not in arguments:
            raise ValueError(
                f"Invalid attribute `{attribute}` for class `{cls}`{message}; "
...
def is_instance_or_subclass(thing, klass):
    ...
    """
    return isinstance(thing, klass) or (
        inspect.isclass(thing) and issubclass(thing, klass)
    )
