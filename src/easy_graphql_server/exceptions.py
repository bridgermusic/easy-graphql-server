...
def graphqlerror__init__(
    ...
    if original_error is None or not isinstance(original_error, BaseError):
        ...
        possible_errors = (
            ...
            ("UNKNOWN_TYPE", r"^Unknown type '(?P<UNKNOWN_TYPE>.*?)'\."),
        )
        ...
        if type_ in ("UNEXPECTED_QUERIED_FIELD", "METHOD_NOT_FOUND"):
            if "suggestions" in payload:
                payload["suggestions"] = re.findall(
                    r"'([^']+)'", payload["suggestions"]
                )
            else:
                payload["suggestions"] = []
        ...
        if isinstance(exception, Exception):
            ...
            payload = {
                "class": (
                    ...
                    else f"{exception_class.__module__}.{exception_class.__qualname__}"
                ),
