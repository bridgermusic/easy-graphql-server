...
class ModelManager:
    ...
    def get_filters(self, mapping=None, prefix=""):
        ...
        if mapping is None:
            mapping = self.model_config.get_type_mapping(
                operation=Operation.READ, with_custom_fields=False
            )
        # browse all fields
        for field_name, graphql_type in mapping.items():
            ...
            if isinstance(graphql_type, (dict, list)):
                submapping = (
                    graphql_type[0] if isinstance(graphql_type, list) else graphql_type
                )
            ...
            # value field
                ...
                for lookup_name, lookup_graphql_type in LOOKUPS.get(
                    graphql_type, {}
                ).items():
                    if not self.model_config.is_lookup_allowed(lookup_name):
                        continue
                    filters[
                        f"{prefixed_field_name}__{lookup_name}"
                    ] = lookup_graphql_type
                    ...
                    if (
                        graphql_type in date_time_types
                        and lookup_graphql_type == graphql_types.Int
                    ):
                        int_lookups = LOOKUPS[graphql_types.Int]
                        for (
                            int_lookup_name,
                            int_lookup_graphql_type,
                        ) in int_lookups.items():
                ...
                if self.model_config.is_lookup_allowed("in"):
                    filters[f"{prefixed_field_name}__in"] = graphql_types.List(
                        graphql_type
                    )
    ...
    def update_one(
        self, authenticated_user, graphql_path, graphql_selection, _=None, **filters
    ):
        """
    ...
    def _read_custom_fields(self, instance, authenticated_user, graphql_selection):
        result = {}
        for custom_field in self.model_config.custom_fields:
            if custom_field.name in graphql_selection and custom_field.can_perfom(
                Operation.READ
            ):
