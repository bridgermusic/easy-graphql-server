...
from .. import conversion, exceptions, graphql_types
...
# pylint: disable=protected-access,too-many-locals,too-many-branches,inconsistent-return-statements
class DjangoModelManager(ModelManager):
    ...
    def get_fields_info(self):
        ...
        for field in self.orm_model._meta.fields:  # pylint: disable=protected-access
            ...
            if isinstance(field, django.db.models.fields.related.ForeignKey):
                ...
                # value field
                fields_info.value[field.attname] = self._to_graphql_type_from_field(
                    related_field
                )
                # foreign field
        ...
        for (
            related
        ) in self.orm_model._meta.related_objects:  # pylint: disable=protected-access
    ...
    def create_one(
        ...
        **data,
        ...
        custom_fields_data = self._extract_custom_fields_data(
            operation=Operation.CREATE, data=data
        )
        # instance itself
        ...
        # pre-save trigger
        self.model_config.on_before_operation(
            instance, authenticated_user, Operation.CREATE, data, depth
        )
        # enforce permissions
        ...
        try:
            ...
            reraise_from_django_validation_error(graphql_path, exception)
        # related data
        ...
        try:
            ...
            reraise_from_django_validation_error(graphql_path, exception)
        # post-save trigger
        self.model_config.on_after_operation(
            instance, authenticated_user, Operation.CREATE, data, depth
        )
        # result
    ...
    def update_one(
        ...
        **filters,
        ...
        # pre-update trigger
        self.model_config.on_before_operation(
            instance, authenticated_user, Operation.UPDATE, data, depth
        )
        # related things
        related_data = {}
        for field_name in list(data.keys()):
            ...
            if field_name in self.fields_info.foreign:
                ...
                if child_data is not None:
                    ...
                    if child_identifier is None:
                        data[
                            field_name
                        ] = child_model_config.orm_model_manager.create_one(
                            authenticated_user=authenticated_user,
                            graphql_path=graphql_path + [field_name],
                            depth=depth + 1,
                            **child_data,
                    ...
                    # if identifier provided, update existing instance
                        ...
                        data[
                            field_name
                        ] = child_model_config.orm_model_manager.update_one(
                            authenticated_user=authenticated_user,
                            graphql_path=graphql_path + [field_name],
                            _=child_data,
                            depth=depth + 1,
            ...
            for child_index, child_data in enumerate(children_data):
                ...
                if child_identifier is None:
                    child_instance = child_model_config.orm_model_manager.create_one(
                        ...
                        depth=depth + 1,
                        **dict(
                            {related_field.value_field_name: instance.pk}, **child_data
                        ),
                    )
    ...
    def delete_one(
        self, authenticated_user, graphql_path, graphql_selection, **filters
    ):
        instance = self._read_one(
            graphql_selection=graphql_selection,
            authenticated_user=authenticated_user,
        ...
        if self.fields_info.primary in graphql_selection:
            pre_result = {
                self.fields_info.primary: getattr(instance, self.fields_info.primary)
            }
        else:
            pre_result = {}
        # actually perform operations
        self.model_config.on_before_operation(
            instance, authenticated_user, Operation.DELETE, None, 0
        )
        instance.delete()
        self.model_config.on_after_operation(
            instance, authenticated_user, Operation.DELETE, None, 0
    ...
    def _read(self, graphql_selection, authenticated_user, **filters):
        ...
        queryset = self.build_queryset(
            graphql_selection=graphql_selection, authenticated_user=authenticated_user
        ).filter(**filters)
        # filter queryset, depending on model config
        queryset = self.model_config.filter_for_user(
            queryset=queryset, authenticated_user=authenticated_user
        )
    ...
    def _instance_to_dict(
        ...
        # pre-read trigger
        self.model_config.on_before_operation(
            instance, authenticated_user, Operation.READ, None, depth
        )
            ...
            # related field
                ...
                if len(children_instances) == 0:
                    ...
                    model_manager = self.model_config.schema.get_model_config(
                        orm_model=children_instances[0].__class__
                    ).orm_model_manager
                    result[field_name] = [
                        ...
                        for child_index, child_instance in enumerate(children_instances)
                    ]
            # foreign field
            elif field_value is not None:
                model_manager = self.model_config.schema.get_model_config(
                    orm_model=field_value.__class__
                ).orm_model_manager
                # pylint: disable=protected-access
        ...
        # post-read trigger
        self.model_config.on_after_operation(
            instance, authenticated_user, Operation.READ, result, depth
        )
        return result
    ...
    def build_queryset(self, graphql_selection, authenticated_user):
        ...
        """
        only, prefetch_related, select_related = self.build_queryset_parts(
            graphql_selection=graphql_selection,
            authenticated_user=authenticated_user,
        )
        base_queryset = self.model_config.filter_for_user(
            self.orm_model.objects, authenticated_user
        )
        if self.restrict_queried_fields:
            return (
                base_queryset.only(*only)
                .prefetch_related(*prefetch_related)
                .select_related(*select_related)
            )
        return base_queryset.prefetch_related(*prefetch_related).select_related(
            *select_related
        )

    def build_queryset_parts(
        self, graphql_selection, authenticated_user, field_prefix=""
    ):
        """
        ...
        for field_name, graphql_subselection in graphql_selection.items():
            ...
            if (
                graphql_subselection is None
                and field_name not in self.fields_info.custom
            ):
            ...
            # there is a subselection, and it is a foreign key
                ...
                (
                    foreign_only,
                    foreign_prefetch_related,
                    foreign_select_related,
    ...
    @staticmethod
    def get_sql_log():
        return [query["sql"] for query in list(django.db.connection.queries)]
