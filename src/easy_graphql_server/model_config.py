...
from . import exceptions, graphql_types, introspection
from .conversion import to_graphql_argument, to_graphql_objecttype
...
class ModelConfig:
    ...
    def __init__(
        ...
        restrict_queried_fields=False,
        has_permission=None,
        filter_for_user=None,
        on_before_operation=None,
        on_after_operation=None,
        allowed_lookups=None,
        disallowed_lookups=None,
        custom_fields=None,
        max_depth=None,
        limit=-1,
    ):
            ...
            if model_callback:
                if isinstance(model_callback, (staticmethod, classmethod)):
                    ...
                    def make_callback(model_callback_name):
                        return lambda instance, *args, **kwargs: getattr(
                            instance, model_callback_name
                        )(*args, **kwargs)
        ...
        self.available_operations = {
            ...
            Operation.CREATE: (
                can_write is not False
                and can_create is not False
                and cannot_create is not True
            ),
            Operation.UPDATE: (
                can_write is not False
                and can_update is not False
                and cannot_update is not True
            ),
            Operation.DELETE: (
                can_write is not False
                and can_delete is not False
                and cannot_delete is not True
            ),
        }
        ...
        # name
        self.name = name or schema.case_manager.convert(
            self.orm_model_manager.get_table_name()
        )
    ...
    def expose_methods(self):
        ...
        if self.available_operations[Operation.CREATE]:
            ...
            self.schema.expose_mutation(
                ...
                require_authenticated_user=Operation.CREATE
                in self.require_authenticated_user,
            )
        # expose read methods
        if self.available_operations[Operation.READ]:
            ...
            self.schema.expose_query(
                ...
                output_format=output_type,
                method=self.orm_model_manager.decorate(self.orm_model_manager.read_one),
                pass_graphql_path=True,
                pass_graphql_selection=True,
                pass_authenticated_user=True,
        ...
        if self.available_operations[Operation.UPDATE]:
            ...
            self.schema.expose_mutation(
                ...
                pass_graphql_selection=True,
                pass_authenticated_user=True,
                require_authenticated_user=Operation.UPDATE
                in self.require_authenticated_user,
            )
        ...
        if self.available_operations[Operation.DELETE]:
            ...
            self.schema.expose_mutation(
                ...
                pass_graphql_selection=True,
                pass_authenticated_user=True,
                require_authenticated_user=Operation.DELETE
                in self.require_authenticated_user,
            )
    ...
    def get_type_mapping(
        ...
        if self.max_depth is not None and (
            max_depth is None or self.max_depth < max_depth
        ):
        ...
        if max_depth is None or max_depth > 0:
            for field_name, field in fields_info.linked.items():
                # ensure the field is exposed for this operation
                if operation is not None and not self.can_perform(
                    operation, field_name
                ):
                    continue
                # retrieve other model config
                other_model_config = self.schema.get_model_config(
                    orm_model=field.orm_model
                )
                if other_model_config is None:
                    continue
                # only_when_child_of is important
                if other_model_config.only_when_child_of and not issubclass(
                    self.orm_model_manager.orm_model,
                    other_model_config.only_when_child_of,
                ):
                    continue
        ...
        if operation == Operation.CREATE:
            for field_name, graphql_type in list(mapping.items()):
                if field_name in fields_info.mandatory:
                    if (
                        linked_field is not None
                        and field_name == linked_field.value_field_name
                    ):
                        continue
    ...
    def _make_required(cls, graphql_type):
        ...
        if isinstance(graphql_type, graphql_types.List):
            return graphql_types.NonNull(
                graphql_types.List(graphql_types.NonNull(graphql_type.of_type))
            )
    ...
    def has_permission(self, instance, authenticated_user, operation, data=None):
        ...
        if data is None:
            data = {}
        return all(
            self._call("has_permission", instance, authenticated_user, operation, data)
        )

    def ensure_permission(
        self, instance, authenticated_user, operation, data=None, graphql_path=None
    ):
        """
    ...
    def on_before_operation(
        self, instance, authenticated_user, operation, data=None, depth=0
    ):
        """
        ...
        if not isinstance(instance, self.orm_model_manager.orm_model):
            raise Exception(
                f"Unexpected instance {instance}, should be of type {self.orm_model_manager.orm_model}"
            )
        return list(
            self._call(
                "on_before_operation",
