...
from graphql.type.definition import GraphQLInputField, GraphQLType  # pylint: disable=

from . import graphql_types, introspection, types
...
def to_graphql_enum_from_choices(
    prefix, choices, description=None, capitalize=True, schema=None
):
    ...
    with _enums_cache_lock:
        if prefix in _enums_cache:
            ...
            graphql_type = _enums_cache[prefix] = GraphQLEnumType(
                f"{prefix}__enum_type",
                {
...
def to_graphql_type(type_, prefix, for_input=False, schema=None):
    ...
    if inspect.isclass(type_):
        ...
        if issubclass(type_, enum.Enum):
            return to_graphql_enum_from_choices(
                prefix=prefix,
                choices=[
                    (key, value.value) for key, value in type_.__members__.items()
                ],
                schema=schema,
            )
    # list
    ...
    if isinstance(type_, types.ModelInterface):
        ...
        if model_config is None:
            raise ValueError(
                f"No model in schema with name `{type_.model_name}` "
                "(consider changing the order of expositions declaration)"
            )
        mapping = model_config.get_type_mapping(
            operation=type_.operation, with_custom_fields=True
        )
        ...
        return to_graphql_type(
            type_=mapping, prefix=prefix, for_input=for_input, schema=schema
        )
    ...
    if isinstance(type_, types.ModelField):
        ...
        try:
            ...
            raise ValueError(
                ...
                f"for model named `{type_.model_name}`"
            ) from error
        return to_graphql_type(
            type_=mapping, prefix=prefix, for_input=for_input, schema=schema
        )
    ...
    if isinstance(type_, types.Model):
        raise ValueError(
            "Could not convert `easy_graphql_server.Model` instance "
...
def to_graphql_argument(type_, prefix, schema=None):
    ...
    # otherwise
    raise ValueError(
        f"Could not convert `{type_}` of type `{type(type_)}` to GraphQL argument, "
