...
from graphql.language.ast import (  # pylint: disable=no-name-in-module
    StringValueNode,
    ValueNode,
)
...
def parse_decimal_value(input_value: Any) -> decimal.Decimal:
    ...
    try:
        ...
        raise ValueError(
            f"Cannot parse Decimal from: {repr(input_value)}, got: {error}"
        ) from error
...
def parse_decimal_literal(
    ...
    if not isinstance(value_node, StringValueNode):
        raise ValueError(
            "Decimal should be represented as a string in input: "
...
def parse_datetime_value(input_value: Any) -> datetime.datetime:
    ...
    try:
        ...
        raise ValueError(
            f"Cannot parse DateTime from: {repr(input_value)}, got: {error}"
        ) from error
...
def parse_datetime_literal(
    ...
    if not isinstance(value_node, StringValueNode):
        raise ValueError(
            "DateTime should be represented as a string in input: "
            + print_ast(value_node),
            value_node,
        )
...
def parse_date_value(input_value: Any) -> datetime.date:
    ...
    try:
        ...
        raise ValueError(
            f"Cannot parse Date from: {repr(input_value)}, got: {error}"
        ) from error
...
def parse_time_value(input_value: Any) -> datetime.time:
    ...
    try:
        ...
        raise ValueError(
            f"Cannot parse Time from: {repr(input_value)}, got: {error}"
        ) from error
...
def parse_jsonstring_value(input_value):
    ...
    try:
        ...
        raise ValueError(
            f"Cannot parse JSONString from: {repr(input_value)}, got: {error}"
        ) from error
...
def parse_jsonstring_literal(value_node, _variables=None):
    ...
    if not isinstance(value_node, StringValueNode):
        raise ValueError(
            "JSONString should be represented as a string in input: "
            + print_ast(value_node),
            value_node,
        )
