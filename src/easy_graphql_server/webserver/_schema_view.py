...
class SchemaView:
    ...
    def _must_serve_graphiql(self, headers):
        ...
        json_priority = (
            len(accepted_content_types)
            - accepted_content_types.index("application/json")
    ...
    def compute_response(self, method, headers, body, query, authenticated_user):
        ...
        if method == "POST":
            ...
            try:
                ...
                return {
                    "errors": [
                        {
                            "message": f"HTTP request body is not valid JSON: {error}",
                        }
                    ]
                }, 400
            if not isinstance(data, dict):
                return {
                    "errors": [
                        {
                            "message": "HTTP request body should be formatted as a JSON object",
                        }
                    ]
                }, 400
        elif method == "GET":
            ...
            try:
                ...
                return {
                    "errors": [
                        {
                            "message": f"Parameter `variables` is not valid JSON: {error}",
                        }
                    ]
                }, 400
        else:
            return {
                "errors": [
                    {
                        "message": f"Method {method} not allowed, only GET and POST are supported",
                    }
                ]
            }, 405
        ...
        if not isinstance(query, str):
            return {
                "errors": [
                    {
                        "message": 'Required parameter "query" should be a string',
                    }
                ]
            }, 400
        ...
        if not isinstance(variables, dict):
            return {
                "errors": [
                    {
                        "message": 'Optional parameter "variables" should be an mapping',
                    }
                ]
            }, 400
        # extract & validate string query
        ...
        if operation_name is not None and not isinstance(operation_name, str):
            return {
                "errors": [
                    {
                        "message": 'Optional parameter "operationName" should be a string',
                    }
                ]
            }, 400
