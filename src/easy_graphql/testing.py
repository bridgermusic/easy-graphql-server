...
DEFAULT_BASE_TEST_CLASS = unittest.TestCase


def generate_testcase(schema, graphql_path, base_test_class=DEFAULT_BASE_TEST_CLASS, django_environment=False):
    ...
    class TestCase(base_test_class):
        ...
        def _replace_extension(path, new_extension):
            return re.sub(r'\.\w+$', f'.{new_extension}', graphql_path)
        @classmethod
        def _iterate_data(cls, graphql_path):
            ...
            if os.path.isfile(sql_path):
                sql_list = open(sql_path, 'rt', encoding='utf-8').read().split(';;')
            else:
        ...
        def run_test(self, *args, **kwargs): # pylint: disable=C0103,W0613
            ...
            if os.path.isfile(setup_script_path):
                spec = importlib.util.spec_from_file_location(f'setup_{class_name}', setup_script_path)
                setup_module = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(setup_module)
            # actually run the test step(s)
            for index, (graphql, json_, sql) in self._iterate_data(graphql_path):
                # compute output
                json_result = schema.execute(graphql, dict_output=True)
                try:
...
def generate_testcases(schema, path, base_test_class=DEFAULT_BASE_TEST_CLASS):
