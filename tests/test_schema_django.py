...
from .testapp.schema1 import schema
...
class SchemaTest(unittest.TestCase):
    ...
    def _search_schema(self, root=None, **criteria):
        ...
        if isinstance(root, list):
            ...
            if all(
                key in root and root[key] == value for key, value in criteria.items()
            ):
                yield root
    ...
    def test_enum_names(self):
        ...
        create_person_gender_arg = self._get_from_schema(
            root=create_person["args"], name="gender"
        )
        ...
        update_person_underscore_arg = self._get_from_schema(
            root=update_person["args"], name="_"
        )
        ...
        update_person_gender_arg = self._get_from_schema(
            root=update_person_underscore_input_type, name="gender"
        )
        ...
        create_house_tenants_input_type = self._get_from_schema(
            root=create_house, name="tenants"
        )["type"]["ofType"]
        create_house_owner_input_type = self._get_from_schema(
            root=create_house, name="owner"
        )["type"]
        for input_type_reference in (
            create_house_tenants_input_type,
            create_house_owner_input_type,
        ):
            input_type = self._get_from_schema(
                kind="INPUT_OBJECT", interfaces=None, name=input_type_reference["name"]
            )
        ...
        for method_name in (
            ...
            for attribute in ("owner", "tenants"):
                method_person_output_type = self._get_from_schema(
                    method_output_type, name=attribute
                )
